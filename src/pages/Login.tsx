      ...
      >
        ...
        <div className="space-y-12">
          ...
          <button
            ...
            className={`w-full h-16 bg-white border border-[#2D2926]/10 flex items-center justify-center gap-4 rounded-full font-bold text-[#2D2926] transition-all hover:border-[#2D2926] hover:shadow-2xl active:scale-95 group shadow-sm ${isAuthenticating ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <div className="w-6 h-6 flex items-center justify-center grayscale group-hover:grayscale-0 transition-all">
              {isAuthenticating ? (
                <div className="w-5 h-5 border-2 border-[#2D2926]/20 border-t-[#2D2926] rounded-full animate-spin"></div>
              ) : (
            ...
            <span className="text-[10px] uppercase tracking-widest font-bold">
              {isAuthenticating ? 'Synchronizing...' : 'Authenticate via Google'}
            </span>
          </button>
